...
// Data & Generators
import transportVocabFallback from '../data/transport_vocab.json';
import { generateTransportQuestions, TRANSPORT_QUESTION_TYPES } from '../utils/transportQuestionGenerator';

const { width, height } = Dimensions.get('window');
const LESSON_ID = 'intermediate_5_transport';
const LESSON_KEY = 'intermediate_5_transport';
const CATEGORY = 'thai-transport';
...
const normalizeTransportItem = (doc) => ({
  id: doc.id || doc._id || `transport_${uid()}`,
  thai: doc.thai || '',
  roman: doc.roman || '',
  en: doc.en || '',
  emoji: doc.emoji || '🚗',
...
const getHintText = (type) => {
  switch (type) {
    case TRANSPORT_QUESTION_TYPES.LISTEN_CHOOSE:
      return 'แตะปุ่มลำโพงเพื่อฟังซ้ำ แล้วเลือกยานพาหนะที่ได้ยิน';
    case TRANSPORT_QUESTION_TYPES.PICTURE_MATCH:
      return 'ดูรูป/emoji แล้วเลือกชื่อยานพาหนะ';
    ...
    case TRANSPORT_QUESTION_TYPES.ARRANGE_SENTENCE:
...
const getTypeLabel = (type) => {
  switch (type) {
    case TRANSPORT_QUESTION_TYPES.LISTEN_CHOOSE: return 'ฟังเสียง';
    case TRANSPORT_QUESTION_TYPES.PICTURE_MATCH: return 'ดูรูป';
    case TRANSPORT_QUESTION_TYPES.TRANSLATE_MATCH: return 'จับคู่ภาษา';
    case TRANSPORT_QUESTION_TYPES.FILL_BLANK_DIALOG: return 'เติมคำ';
    case TRANSPORT_QUESTION_TYPES.ARRANGE_SENTENCE: return 'เรียงคำ';
    default: return '';
  }
};
...
const checkAnswer = (question, answer) => {
  switch (question.type) {
    ...
    case TRANSPORT_QUESTION_TYPES.TRANSLATE_MATCH:
      return answer && answer.every(pair =>
...
const IntermediateTransportGame = ({ navigation, route }) => {
  const {
    ...
    stageTitle = 'การเดินทาง (Transportation & Movement)',
  ...
  useEffect(() => {
    const loadTransportItems = async () => {
      try {
        const normalizedItems = (transportVocabFallback || [])
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn('Service initialization error:', error?.message);
      }
    })();
    ...
    try {
      ...
      try {
        await gameProgressService.saveGameSession({
          ...
          gameMode: 'intermediate_5_transport',
        ...
        console.warn('Failed to save game session:', sessionError?.message);
      }

      let unlockResult = null;
      if (unlockedNext) {
        try {
          unlockResult = await levelUnlockService.checkAndUnlockNextLevel('level_intermediate_5', {
            accuracy: accuracyPercent,
            score: correctAnswers,
            attempts: 1,
          });
        } catch (unlockError) {
          console.warn('Failed to unlock next level:', unlockError?.message);
        }
      }
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.startContainer}>
          <View style={styles.introCard}>
            ...
            <Text style={styles.startSubtitle}>เรียนรู้การเดินทางและยานพาหนะ</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  statBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.white, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 15, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 2, borderWidth: 1, borderColor: '#F2F2F2' },
  statText: { fontSize: 14, fontWeight: '600', color: COLORS.dark, marginLeft: 5 },
  heartAnimation: { width: 20, height: 20 },
  starAnimation: { width: 20, height: 20 },
  diamondAnimation: { width: 20, height: 20 },
  streakAnimation: { width: 20, height: 20 },
  accuracyContainer: { backgroundColor: 'rgba(255, 128, 0, 0.1)', paddingVertical: 10, paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: '#FFE3CC', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  accuracyLabel: { fontSize: 14, fontWeight: '600', color: COLORS.primary },
  accuracyPercent: { fontSize: 18, fontWeight: 'bold', color: COLORS.primary },
  ...
  pairPreview: { alignSelf: 'center', flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 18, borderWidth: 1, borderColor: '#F2F2F2', marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 },
  pairPreviewText: { fontSize: 14, fontWeight: '700', color: COLORS.dark },
  pairArrow: { fontSize: 16, marginHorizontal: 8, color: COLORS.primary, fontWeight: '900' },
