...
const getHintText = (type) => {
  switch (type) {
    ...
    case QUESTION_TYPES.FILL_CONTEXT:
      return 'เลือกสำนวนที่เหมาะสมกับบริบท';
...
// Question Generators
const makeListenMeaning = (idiom, pool) => {
  const correct = idiom.meaningTH;
  const wrong = pool
    .filter(p => p.id !== idiom.id)
    .map(p => p.meaningTH)
    .slice(0, 3);
  const choices = shuffle([correct, ...wrong]).slice(0, 4);
  
  return {
    id: `lm_${idiom.id}_${uid()}`,
    type: QUESTION_TYPES.LISTEN_MEANING,
    instruction: 'ฟังเสียงแล้วเลือกความหมายที่ถูกต้อง',
...
const makeListenUsage = (idiom, pool) => {
  const choices = shuffle([
    { id: 1, text: idiom.thai, isCorrect: true },
    { id: 2, text: pick(pool.filter(p => p.id !== idiom.id))?.thai || 'สำนวนอื่น', isCorrect: false },
    { id: 3, text: pick(pool.filter(p => p.id !== idiom.id))?.thai || 'สำนวนอื่น', isCorrect: false },
    { id: 4, text: pick(pool.filter(p => p.id !== idiom.id))?.thai || 'สำนวนอื่น', isCorrect: false },
  ]);
  
  return {
    id: `lu_${idiom.id}_${uid()}`,
    type: QUESTION_TYPES.LISTEN_USAGE,
    instruction: 'ฟังประโยคตัวอย่างแล้วเลือกสำนวนที่เหมาะสม',
    questionText: 'แตะปุ่มลำโพงเพื่อฟัง',
    audioText: idiom.exampleTH || idiom.thai,
    correctText: idiom.thai,
    choices,
  };
};

const makeMatchIdiomMeaning = (idiom, pool) => {
  const similar = pool.filter(p => p.id !== idiom.id).slice(0, 3);
  const batch = [idiom, ...similar];
  
  const leftItems = shuffle(batch).map((item, idx) => ({
    id: `left_${idx + 1}`,
    text: item.thai,
    correctMatch: item.meaningTH,
  }));
  ...
  return {
    id: `mm_${idiom.id}_${uid()}`,
    type: QUESTION_TYPES.MATCH_IDIOM_MEANING,
    instruction: 'จับคู่สำนวนไทยกับความหมายที่ถูกต้อง',
    leftItems,
    rightItems,
  };
};

const makeFillContext = (idiom) => {
  const choices = shuffle([
    { id: 1, text: idiom.thai, isCorrect: true },
    { id: 2, text: 'น้ำขึ้นให้รีบตัก', isCorrect: false },
    { id: 3, text: 'ขี่ช้างจับตั๊กแตน', isCorrect: false },
  ]);
  
  return {
    id: `fc_${idiom.id}_${uid()}`,
    type: QUESTION_TYPES.FILL_CONTEXT,
    instruction: 'เลือกสำนวนที่เหมาะสมกับบริบท',
    questionText: `ช่วงนี้ลูกค้าสนใจเยอะ _____ เพื่อรับคำสั่งให้ได้!`,
    correctText: idiom.thai,
    choices,
  };
};

const makeArrangeIdiom = (idiom) => {
  const words = idiom.thai.split(/\s+/).filter(w => w.trim());
  if (words.length < 2) return null;
  
  const distractors = ['ครับ', 'ค่ะ', 'ไหม'];
  const allParts = shuffle([...words, ...distractors]);
  
  return {
    id: `arr_${idiom.id}_${uid()}`,
    type: QUESTION_TYPES.ARRANGE_IDIOM,
    instruction: 'เรียงคำให้เป็นสำนวนไทยที่ถูกต้อง',
    correctOrder: words,
    allParts,
  };
};

const makeParaphraseSelect = (idiom) => {
  const choices = shuffle([
    { id: 1, text: idiom.meaningTH, isCorrect: true },
    { id: 2, text: 'ทำเรื่องยากเกินความจำเป็น', isCorrect: false },
    { id: 3, text: 'รักษาบัวไม่ให้ช้ำ', isCorrect: false },
    { id: 4, text: 'ใช้วิธีเกินความเหมาะสม', isCorrect: false },
  ]);
  
  return {
    id: `par_${idiom.id}_${uid()}`,
    type: QUESTION_TYPES.PARAPHRASE_SELECT,
    instruction: 'เลือกถอดความที่แม่นยำที่สุด',
    questionText: `"${idiom.thai}" หมายถึง`,
    correctText: idiom.meaningTH,
    choices,
  };
};

const makeTrueFalse = (idiom, isTrue) => {
  const statement = isTrue
    ? `"${idiom.thai}" แปลว่า ${idiom.meaningTH}`
    : `"${idiom.thai}" แปลว่า "ทำเรื่องเล็กเป็นเรื่องใหญ่"`;
  ...
  return {
    id: `tf_${idiom.id}_${uid()}`,
    type: QUESTION_TYPES.TRUE_FALSE_EXPLANATION,
    instruction: 'ตัดสินว่าข้อความถูกหรือผิด',
    statement,
...
// Generate questions (14 total)
const generateIdiomsQuestions = (pool) => {
  ...
  for (let i = 0; i < 3 && pool.length > usedIds.size; i++) {
    ...
    const idiom = pick(available);
    usedIds.add(idiom.id);
    const q = makeListenMeaning(idiom, pool);
  ...
  for (let i = 0; i < 2 && pool.length > usedIds.size; i++) {
    ...
    const idiom = pick(available);
    usedIds.add(idiom.id);
    const q = makeListenUsage(idiom, pool);
    if (q) questions.push(q);
  }
  
  // MATCH_IDIOM_MEANING × 2
  for (let i = 0; i < 2 && pool.length > usedIds.size; i++) {
    const available = pool.filter(p => !usedIds.has(p.id));
    if (available.length === 0) break;
    const idiom = pick(available);
    usedIds.add(idiom.id);
    const q = makeMatchIdiomMeaning(idiom, pool);
    if (q) questions.push(q);
  }
  ...
  for (let i = 0; i < 2 && pool.length > usedIds.size; i++) {
    ...
    const idiom = pick(available);
    usedIds.add(idiom.id);
    const q = makeArrangeIdiom(idiom);
    if (q) questions.push(q);
  }
  ...
  for (let i = 0; i < 2 && pool.length > usedIds.size; i++) {
    ...
    const idiom = pick(available);
    usedIds.add(idiom.id);
    const q = makeParaphraseSelect(idiom);
    if (q) questions.push(q);
  }
  ...
  if (pool.length > usedIds.size) {
    ...
    if (available.length > 0) {
      const idiom = pick(available);
      usedIds.add(idiom.id);
      const q = makeTrueFalse(idiom, Math.random() > 0.5);
      if (q) questions.push(q);
    }
  }
...
const Advanced1IdiomsGame = ({ navigation, route }) => {
  const {
    lessonId = LESSON_ID,
  ...
  useEffect(() => {
    const loadIdioms = async () => {
      try {
        ...
        // Generate questions
        const generatedQuestions = generateIdiomsQuestions(normalizedIdioms);
...
export default Advanced1IdiomsGame;
